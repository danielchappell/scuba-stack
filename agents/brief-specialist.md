...
- Write the brief to `.scuba/briefs/<epic>.html` as a single self-contained file, updating it in place at v2. Write it with the {{target.fileEditTools}}, never with Bash heredocs (`cat > f << EOF`) — heredocs silently truncate on a broken shell, landing a partial file that reports success. After writing, you may sanity-check the byte/line count, but never fall back to a heredoc.
