...
In both modes, prove the fix with a regression test first: write the failing repro, watch it RED, fix, watch it GREEN, then revert and confirm it goes RED again before restoring — per `test-driven-development` (and `adversarial-review` for the RED→GREEN non-vacuous discipline). Pin the test to the behavior that must hold, not to your specific patch, so it survives the refactor instead of locking a bolt-on in. A green unit test proves a branch runs; it does not prove the bug is gone — verify on the same surface you reproduced it on. Order the commits so the red repro is recorded first and the fix sits on top of it; the history should read as problem-then-cure. In a PR hardening round where the steward requires one cohesive fix commit, keep the RED/GREEN evidence in the control-plane report instead of splitting the round into a drip of commits.
...
For a PR hardening round, read the entire worklist before touching code: the external comments, the bug-class hunter, and the adjacent-surface hunter are symptoms of one not-yet-hardened system state. Name the shared root causes or violated invariants first, then design the smallest full-system repair that closes the bug class and adjacent fallout. Do not produce comment-by-comment conditional patches. The default deliverable is one cohesive fix commit for the round; splitting is an exception the steward must approve and record.
