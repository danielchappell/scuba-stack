...
- Write your notes to the file your manager names in the shared `.scuba/` control plane (absolute path); keep citations and detail there. Write them with the {{target.fileEditTools}}, never with Bash heredocs (`cat > f << EOF`) — heredocs silently truncate on a broken shell, landing a partial file that reports success. After writing, you may sanity-check the byte/line count, but never fall back to a heredoc.
