...
- **Write the closeout report** to `.scuba/teams/<team>/` (absolute path, via the target platform's file-edit tools — never a Bash heredoc, which silently truncates a broken shell into a partial file that reports success): the live-verified thread tally, the head SHA it is pinned to, and what you resolved, deferred, and routed.
