...
Project-specific only. The Scuba Stack rules live globally at {{target.userScopeHome}} — do not repeat them here.
...
- Re-trigger after a fix: <e.g. push new commits, or {{target.externalReviewerRetriggerExample}}>
- Typical latency: <so the internal swarm and reconcile are timed against it>
- **Reviews non-`main`-base PRs too?** The integration-branch model puts story PRs on a non-default base. Confirm your reviewer comments on those ({{target.externalReviewerNonMainDefault}}) and do **not** restrict it to the default branch, or story PRs ship unreviewed.
